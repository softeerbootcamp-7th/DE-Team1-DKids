...
"""Fetch Hyunki Market detail pages and write CSV parts to S3."""
...
DEFAULT_RESULT_PREFIX = "maintenance_parts/hyunki_market/results"
DEFAULT_SKIP_PREFIX = "maintenance_parts/hyunki_market/skipped"
DEFAULT_LOG_LEVEL = "INFO"
DEFAULT_TIMEOUT_SECONDS = 20
FIELDNAMES = ["extracted_at", "name", "part_no", "price", "car_type"]
...
def extract_fields(html_text: str) -> Dict[str, str]:
    """Extract fields from a Hyunki Market detail HTML page.
    ...
    name = find_first(
        html_text,
        [
            r"부품명</span>\s*</th>\s*<td>\s*<span[^>]*>(.*?)</span>",
            r"<h2 class=\"titleFont\"[^>]*>(.*?)</h2>",
    ...
    price = find_first(
        ...
        [
            ...
            r"판매가</span>\s*</th>\s*<td>\s*<span[^>]*>\s*<strong[^>]*>(.*?)</strong>",
        ],
    )
    if not price:
    ...
    part_no = find_first(
        html_text,
        [
            r"부품번호</span>\s*</th>\s*<td>\s*<span[^>]*>(.*?)</span>",
        ],
    )
    ...
    car_type = find_first(
        html_text,
        [
            r"xans-product-additional\s+simple_desc_top[^>]*>\s*(.*?)\s*</div>",
...
def handler(event, context):
    """AWS Lambda handler to process Hyunki Market detail pages.
